...
// Enabling raw mode that can display characters directly while writing,
// oppositely to canonical mode that waits for a specified  signal to print all typed characters.
...
int editorRowCxToRx(erow *row, int cx)
{
  ...
  {
    if (row->chars[j] == '\t')
      rx += (MAHO_TAB_STOP - 1) - (rx % MAHO_TAB_STOP);
    rx++;
  }
  return rx;
}
...
void editorUpdateRow(erow *row)
{
  ...
  free(row->render);
  row->render = malloc(row->size + tabs * (MAHO_TAB_STOP - 1) + 1);
...
// For reallocating the memory block to the size of the current string,
// and to copy it after the end of the current data in the buffer,
// later update the length and and the pointer of abuf with the new values
